...
const About = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const img = sectionRef.current.querySelector('.about-img-container');
      const content = sectionRef.current.querySelector('.about-content');
      const items = sectionRef.current.querySelectorAll('.credential-item');

      gsap.set([img, content], { opacity: 0, y: 40 });
      ...
      tl.to(img,     { opacity: 1, y: 0, duration: 0.7, ease: 'power2.out' })
        .to(content, { opacity: 1, y: 0, duration: 0.7, ease: 'power2.out' }, '-=0.4')
        .to(items,   { opacity: 1, y: 0, duration: 0.5, stagger: 0.07, ease: 'power2.out' }, '-=0.4');
    }, sectionRef);
  ...
  return (
    <section id="a-propos" ref={sectionRef} className="section-padding about">
      <div className="container" style={{ maxWidth: '1200px' }}>
        <div className="about-grid">
          <div className="about-img-container">
            <img src={drPhoto} alt="Dr. Eddassi Noureddine" className="about-img" />
            <div style={{
              ...
              color: '#fff', display: 'flex', flexDirection: 'column', gap: '0.5rem',
            }}>
              <h3 style={{ fontSize: '1.8rem', fontWeight: 800, letterSpacing: '-0.5px', marginBottom: 0 }}>
                Dr. EDDASSI Noureddine
              </h3>
              <p style={{ color: 'var(--primary-orange)', fontWeight: 700, fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '3px', margin: 0 }}>
                Ophtalmologiste Expert
              </p>
            </div>
          </div>
